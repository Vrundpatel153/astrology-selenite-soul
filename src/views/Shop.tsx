...
export default function Shop() {
  ...
  const concernParam = useMemo(() => new URLSearchParams(search).get("concern"), [search]);
  const astrologyParam = useMemo(() => new URLSearchParams(search).get("astrology"), [search]);

  const [selectedCategories, setSelectedCategories] = useState<string[]>(categoryParam ? [categoryParam] : []);
  const [selectedGemstones, setSelectedGemstones] = useState<string[]>([]);
  const [selectedConcerns, setSelectedConcerns] = useState<string[]>(concernParam ? [concernParam] : []);
  const [selectedZodiacs, setSelectedZodiacs] = useState<string[]>(astrologyParam ? [astrologyParam] : []);
  ...
  useEffect(() => {
    ...
    setSelectedZodiacs(astrologyParam ? [astrologyParam] : []);
    setSelectedGemstones([]);
  }, [categoryParam, concernParam, astrologyParam]);
  ...
  const toggleZodiac = (id: string) =>
    setSelectedZodiacs(prev => prev.includes(id) ? prev.filter(z => z !== id) : [...prev, id]);

  const baseProducts = useMemo(() => {
    if (filterParam === "new") return newArrivals;
    if (filterParam === "best") return bestSellers;
    if (filterParam === "gifting") {
      // Filter premium combo items or raw sets as gift options
      return products.filter(p => p.price >= 1299 || p.name.toLowerCase().includes("combo") || p.name.toLowerCase().includes("yog"));
    }
    return products;
  }, [filterParam]);
  ...
  const filteredProducts = useMemo(() => baseProducts.filter(p => {
    ...
    const concernMatch = selectedConcerns.length === 0 || (p.concern && selectedConcerns.some(c => p.concern!.includes(c)));
    const zodiacMatch = selectedZodiacs.length === 0 || (() => {
      const activeCrystals = selectedZodiacs.map(z => {
        const signData = zodiacSigns.find(zs => zs.sign.toLowerCase() === z.toLowerCase());
        return signData ? signData.crystal : null;
      }).filter(Boolean) as string[];
      return p.gemstone && activeCrystals.some(ac => p.gemstone!.includes(ac));
    })();
    return catMatch && gemMatch && concernMatch && zodiacMatch;
  }), [baseProducts, selectedCategories, selectedGemstones, selectedConcerns, selectedZodiacs]);
    ...
    <div className="min-h-screen bg-[#fdf8f4] text-[#2a1f1a]" data-testid="page-shop">
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <aside className="hidden md:block w-[260px] lg:w-[280px] shrink-0 border-r border-[#e8d9cf] bg-[#f7f1ec]/50 min-h-screen p-6">
          ...
          <div className="h-px bg-[#e8d9cf] my-8" />

          <ScrollReveal direction="left" delay={0.25}>
            <div>
              <h3 className="text-xs font-bold uppercase tracking-widest text-[#2a1f1a] mb-4">Shop by Concern</h3>
              <div className="space-y-3">
                {concerns.map((con, i) => (
                  <motion.label
                    key={con.id}
                  ...
                  >
                    <motion.div
                      className={`w-4 h-4 border flex items-center justify-center transition-colors ${selectedConcerns.includes(con.id) ? "bg-[#2a1f1a] border-[#2a1f1a]" : "border-[#e8d9cf] group-hover:border-[#2a1f1a]"}`}
                      onClick={() => toggleConcern(con.id)}
                      whileTap={{ scale: 0.85 }}
                    >
                      {selectedConcerns.includes(con.id) && (
                        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-2 h-2 bg-white" />
                      )}
                    </motion.div>
                    <span className="text-sm text-[#2a1f1a]">{con.name}</span>
                  </motion.label>
                ))}
              </div>
            </div>
          ...
          <ScrollReveal direction="left" delay={0.3}>
            <div>
              <h3 className="text-xs font-bold uppercase tracking-widest text-[#2a1f1a] mb-4">Astrology (Zodiac)</h3>
              <div className="space-y-3 max-h-[220px] overflow-y-auto pr-2 scrollbar-thin">
                {zodiacSigns.map((zod, i) => (
                  <motion.label
                    key={zod.sign}
                    className="flex items-center gap-3 cursor-pointer group"
                    initial={{ opacity: 0, x: -8 }}
                  ...
                  >
                    <motion.div
                      className={`w-4 h-4 border flex items-center justify-center transition-colors ${selectedZodiacs.includes(zod.sign) ? "bg-[#2a1f1a] border-[#2a1f1a]" : "border-[#e8d9cf] group-hover:border-[#2a1f1a]"}`}
                      onClick={() => toggleZodiac(zod.sign)}
                      whileTap={{ scale: 0.85 }}
                    >
                      {selectedZodiacs.includes(zod.sign) && (
                        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-2 h-2 bg-white" />
                      )}
                    </motion.div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            ...
            {filteredProducts.length === 0 && (
              ...
              >
                ...
                <motion.button
                  onClick={() => { setSelectedCategories([]); setSelectedGemstones([]); setSelectedConcerns([]); setSelectedZodiacs([]); }}
        ...
        {mobileFilterOpen && (
          <>
            ...
            >
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    {concerns.map(con => (
                      ...
                      >
                        <motion.div
                          className={`w-5 h-5 border flex items-center justify-center ${selectedConcerns.includes(con.id) ? "bg-[#2a1f1a] border-[#2a1f1a]" : "border-[#e8d9cf]"}`}
                          whileTap={{ scale: 0.85 }}
                        >
                          {selectedConcerns.includes(con.id) && (
                            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-2.5 h-2.5 bg-white" />
                        ...
                        <span className="text-base">{con.name}</span>
                      </motion.label>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    {zodiacSigns.map(zod => (
                      ...
                      >
                        <motion.div
                          className={`w-5 h-5 border flex items-center justify-center ${selectedZodiacs.includes(zod.sign) ? "bg-[#2a1f1a] border-[#2a1f1a]" : "border-[#e8d9cf]"}`}
                          whileTap={{ scale: 0.85 }}
                        >
                          {selectedZodiacs.includes(zod.sign) && (
                            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-2.5 h-2.5 bg-white" />
              ...
              <div className="p-4 bg-white border-t border-[#e8d9cf] flex gap-3 shrink-0">
                <motion.button

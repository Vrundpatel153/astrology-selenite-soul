...
export default function NewArrivals() {
  ...
  return (
    <section className="py-10 md:py-20 bg-background overflow-hidden" data-testid="section-new-arrivals">
      {/* Header */}
      <motion.div
      ...
      >
        <div>
          <h2 className="text-[32px] md:text-[52px] lg:text-[60px] font-bold text-[#2a1f1a] uppercase tracking-tight leading-none" data-testid="heading-new-arrivals">
            NEW ARRIVALS
          </h2>
        </div>
      </motion.div>
      ...
      <div className="overflow-hidden" ref={emblaRef}>
        <div className="flex">
          {newArrivals.map((product) => (
            ...
            >
              {product.badge && (
                <div className="absolute top-3 right-3 z-10 bg-background text-[#2a1f1a] px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider border border-[#e8d9cf]">
                  {product.badge}
                </div>
              )}
              {/* Image Area */}
